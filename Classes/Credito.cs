...
namespace ExemploPOO1.Classes
{
    public class Credito : Cartao
    {
        ...
        public void Pagar()
        {
            ...
            if (valor > Limite)
            {
                Console.WriteLine("Você nao tem limite suficiente para fazer esta compra!");
            }
            else
            {
                ...
                do
                {
                    ...
                    if (vezes >= 1 && vezes <= 6)
                    {
                        Console.WriteLine($"Você optou por pacelar em {vezes} vezes, dessa forma o juros será de 5% do valor total");
                        Console.WriteLine($"Você pagará {valor * 1.05:C2} em {vezes} vezes de {(valor * 1.05) / vezes:C2}");
                    }
                    else if (vezes >= 7 && vezes <= 12)
                    {
                        Console.WriteLine($"Você optou por pacelar em {vezes} vezes, dessa forma o juros será de 8% do valor total");
